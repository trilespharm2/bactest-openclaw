...
-   **Simulated Trading**: A two-page interface for setting up and conducting simulated trading sessions (stock/options) with a dark-themed, full-screen trading page using TradingView Lightweight Charts. Features include candlestick charts, OHLC bars, timeframe switching, bar navigation, position management, and session persistence. The sim trading result detail trade log for options mode shows enhanced columns: expand arrow, #, Strategy, Qty, Net Premium Entry/Exit, Underlying at Entry, Entry/Exit Time, Duration, Expiration, Exit Reason, and P&L. Each trade row is expandable to show per-leg details (symbol, type, position, strike, entry/exit price, per-leg P&L per contract). CSV download includes all columns plus per-leg data. Exit prices use quantity-weighted averages across partial closes. Data enrichment is handled by `enrichOptionTrade()` in `simulated-trading-script.js`.
