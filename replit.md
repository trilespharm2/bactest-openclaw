...
-   **Exit Conditions**: Stock backtester supports signal-based exit conditions (section [5/8]) in addition to TP/SL/Max Days. Offers Preset (Change %, Gap %, Velocity, etc.) and Custom Builder modes, mirroring the entry conditions UI. All exit mechanisms are optional but at least one must be configured. Custom exit conditions use the same `check_custom_condition()` engine method. Exit condition signals are checked on each candle while holding, after TP/SL but before max_days. Exit condition IDs use `exit-` prefix (e.g., `exit-metric-N`, `exit-left-day-N`).
