...
- **Underlying Price Conditions**: Advanced entry filtering system with technical indicators (SMA, EMA, RSI, MACD) supporting both intraday minute bars and day-level bars. Day candle conditions compare daily OHLC values (e.g., today's open vs yesterday's close) with proper timezone conversion from UTC to Eastern. When Day candle type is selected, entry time auto-locks to 09:30 AM. The stock backtester custom condition builder supports indicator-based conditions on both left and right sides: SMA/EMA (configurable period and series), RSI (configurable period, always uses close), MACD (configurable short/long/signal windows, component selection: histogram/signal/MACD line). The right side also supports a fixed numeric "Value" type for absolute comparisons (e.g., RSI < 30, MACD histogram > 0). Indicators are computed locally from the minute-level OHLCV data using up to 60 days of history, with lookahead bias prevention during intraday iteration.
