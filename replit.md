...
- **Decision Tree Log**: Stock backtest results include a per-day decision log showing every trading day in the period. The backend engine (`backtester_engine_v3_0__6_.py`) builds a `decision_log` alongside trades, tracking for each day: previous close, condition checked, whether condition was met (with price/time), entry/exit details, or skip reason. The frontend renders this as an expandable day-by-day log with color-coded statuses (Entry, Exit, Holding, Skipped).
