...
-   **Input Validation**: Comprehensive client-side validation for all backtester inputs, covering date ranges, symbol formats, indicator bounds, and financial constraints. Error messages with multiple items are displayed as separate lines using safe DOM text nodes (no innerHTML XSS risk). The loading indicator is automatically hidden when errors are shown.
-   **Form Control Deduplication**: Options backtester form controls (radio buttons, strategy select, submit handler) are initialized exclusively in `backtester-script.js` with dataset-based guards to prevent duplicate event listener registration across SPA navigations. Legacy inline handlers in `index.html` (`initOptionsBacktester`) are disabled to prevent conflicting DOM mutations.
