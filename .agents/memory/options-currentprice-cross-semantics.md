...
- `cross_up` = PREVIOUS bar's OPEN below the comparator AND THIS bar's current price (VWAP, falls back to close) at/above the comparator. THIS bar's own open is NOT required to be below the line.
- `cross_down` = mirror (prev open above, VWAP at/below).
- The previous-bar-open gate fails closed: if the previous bar's open or comparator is unavailable, the cross does NOT fire.
- **Why the current-bar-open side check was dropped:** user explicitly decided the current bar's open being below/above the line is not mandatory — only the previous bar's open gates direction, plus the live price crossing. Applies to BOTH the options backtester and the live bot (`bot_executor.py`), kept in sync.
