    ...
    if (data.mode !== 'stock') {
        ...
        (data.trades || []).forEach(function(t) {
            ...
            if (missingLegs.length > 0) {
                var payload = {
                    legs: missingLegs.map(function(l) { return { strike: l.strike, entryPrice: l.entryPrice, type: l.type }; }),
                    underlying_price: t.underlyingAtEntry,
                    entry_timestamp: t.entryTime,
                    expiration_date: t.expiration
                };
                var p = fetch('/api/simulated-trading/compute-greeks', {
                    method: 'POST',
                ...
                .then(function(resp) {
                    if (resp.success && resp.greeks) {
                        resp.greeks.forEach(function(g, idx) {
                            if (g && g.iv !== null && missingLegs[idx]) {
                                missingLegs[idx].iv = g.iv;
                                missingLegs[idx].delta = g.delta;
                                missingLegs[idx].gamma = g.gamma;
                                missingLegs[idx].theta = g.theta;
                                missingLegs[idx].vega = g.vega;
                            }
                        });
                    }
                })
                .catch(function(err) { console.warn('[SimResults] Greeks fetch failed:', err); });
                greeksPromises.push(p);
            }
        });
        if (greeksPromises.length > 0) {
            Promise.all(greeksPromises).then(function() {
                renderTradeLog(data);
            });
        }
    }
