...
// Note: initSubscriptionPage() is called by showPage() when navigating to subscription page
...
function initSubscriptionPage() {
    ...
    // Setup pricing page toggle for logged-out users
    setupPricingPageToggle();
    
    // Only load subscription data if user is authenticated
    if (typeof isAuthenticated !== 'undefined' && isAuthenticated) {
        loadAllSubscriptionData();
    } else {
        // For logged-out users, show free plan defaults
        updateFreeUserDisplay();
    }
...
function setupPricingPageToggle() {
    const pricingToggle = document.getElementById('pricingBillingToggle');
    if (pricingToggle) {
        pricingToggle.addEventListener('change', function() {
            const isAnnual = this.checked;
            const pricingCards = document.querySelectorAll('.pricing-card-page');
            pricingCards.forEach(card => {
                const priceEl = card.querySelector('.price');
                if (priceEl) {
                    const monthly = parseInt(priceEl.dataset.monthly) || 0;
                    const annual = parseInt(priceEl.dataset.annual) || 0;
...
function updateFreeUserDisplay() {
...
// Don't auto-initialize - will be called by showPage when subscription page is active
